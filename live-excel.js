...
const BIG_FIVE_SERVICE_BREAKDOWN_SEED = [
  {
    event: "Welcome Home Sunday",
    year: 2025,
    campus: "BWI",
    services: [
      ["8:30 AM", 1550, 1158],
  ...
  campus.services.map(([service, total, auditorium]) => ({
    event: campus.event,
    eventLabel: campus.event,
    year: campus.year,
    campus: campus.campus,
    service,
    total,
    auditorium,
    source: "WHS 2026 Prep Doc",
  })),
);
...
async function readWorkbookData(token, driveItem) {
  ...
  sheets["Big 5 Service Breakdown"] = await tryReadFirstRange(
    ["Big 5 Service Breakdown", "Big 5 Service Detail", "Big 5 Services", "Big 5 Service Attendance"],
    "A1:Z2000",
  );
...
function buildDashboardData(sheets, driveItem) {
  ...
  const bigFive = extractBigFive(
    ...
    sheets["Big 5 Service Breakdown"],
    campuses,
  );
  ...
  return tableRows(range, ["campus", "ministry"])
    .map((row) => {
      ...
      return {
        ...
        partiallyFilledDate:
          rowDate(row, [
            "partially_filled_date",
            "partial_filled_date",
            "partial_fill_date",
            "date_partially_filled",
            "partially_filled_on",
          ]) ||
          rowText(row, [
            "partially_filled_date",
...
function extractHealthMonths(
  ...
  for (const row of teamLeadVacancies) {
    months.add(row.month);
    months.add(monthKeyFromDateLike(row.vacantSince));
    months.add(monthKeyFromDateLike(row.partiallyFilledDate));
    months.add(monthKeyFromDateLike(row.filledDate));
  }
...
function bigFiveServiceEventKey(value) {
  const normalized = normalizeCompact(value);
...
function bigFiveServiceRecordKey(row) {
  return [
    bigFiveServiceEventKey(row.event),
    row.year,
    normalizeCompact(row.campus),
    normalizeCompact(row.service),
  ].join("::");
}

function mergeBigFiveServiceBreakdown(seedRows, liveRows) {
  const merged = new Map();
  for (const row of seedRows) {
    merged.set(bigFiveServiceRecordKey(row), row);
  }
...
function extractBigFiveServiceBreakdown(range, campuses = []) {
  const rows = tableRows(range, ["event", "campus"]);
  const output = [];

  for (const row of rows) {
    const event = rowText(row, ["event", "big 5 event", "event name"]);
    const yearValue = rowNumber(row, ["year"]);
    const date = rowDate(row, ["date", "service date"], yearValue);
    const year = yearValue || yearFromIso(date);
    const rawCampus = rowText(row, ["campus", "location"]);
    const campus = canonicalCampusName(rawCampus, campuses);
    const service = rowText(row, ["service", "service time", "time", "service label"]);
    const total = rowNumber(row, ["total attendance", "attendance", "total"]);
    const auditorium = rowNumber(row, [
    ...
    if (!event || !year || !campus || !service || (total === null && auditorium === null)) continue;
    if (normalizeHeader(campus)?.startsWith("total") || normalizeHeader(service) === "total") continue;
    ...
    output.push({
      ...
      campus,
      service,
      total: total === null ? null : normalizeValue(total),
      auditorium: auditorium === null ? null : normalizeValue(auditorium),
      source: "Live Excel",
    });
  }
...
function extractBigFive(rawRange, pivotRange, serviceRange = null, campuses = []) {
  ...
  return {
    ...
    serviceBreakdown: mergeBigFiveServiceBreakdown(
      BIG_FIVE_SERVICE_BREAKDOWN_SEED,
      extractBigFiveServiceBreakdown(serviceRange, campuses),
    ),
