...
const els = {
  ...
  cleanTrendTitle: document.querySelector("#cleanTrendTitle"),
  cleanTrendMeta: document.querySelector("#cleanTrendMeta"),
  cleanTrendKpis: document.querySelector("#cleanTrendKpis"),
  cleanTrendChart: document.querySelector("#cleanTrendChart"),
  ...
  bigFiveServicePanel: document.querySelector("#bigFiveServicePanel"),
  bigFiveServiceTitle: document.querySelector("#bigFiveServiceTitle"),
  bigFiveServiceMeta: document.querySelector("#bigFiveServiceMeta"),
  bigFiveServiceSummary: document.querySelector("#bigFiveServiceSummary"),
  bigFiveServiceTableHead: document.querySelector("#bigFiveServiceTableHead"),
  bigFiveServiceTable: document.querySelector("#bigFiveServiceTable"),
...
function cleanTrendCurrentYear() {
  return data.source.latestAttendanceDate?.slice(0, 4) || "2026";
}

function cleanTrendHistoryCampusKeys() {
  const history = data.history?.attendance2025 || {};
  const keys = Object.keys(history);
  if (state.campus === "All Campuses") return keys;
  return keys.filter((campus) => campusNameMatches(campus, state.campus));
}

function aggregateAttendanceSeries(series, campusKeys, sourceLabel) {
  const byDate = new Map();
  for (const campus of campusKeys) {
    for (const point of series?.[campus] || []) {
      if (!point.isSunday || point.value <= 0) continue;
      if (!state.showEvents && point.eventType !== "normal") continue;
      if (!byDate.has(point.date)) {
        byDate.set(point.date, {
          ...
          value: 0,
          event: point.event,
          eventType: point.eventType || "normal",
          isSunday: true,
          sourceLabel,
        });
      }
...
function cleanTrendHistoryPoints() {
  const history = data.history?.attendance2025 || {};
  return aggregateAttendanceSeries(history, cleanTrendHistoryCampusKeys(), "2025 history").filter((point) =>
    point.date.startsWith("2025"),
  );
}

function cleanTrendCurrentPoints() {
  const currentYear = cleanTrendCurrentYear();
  return getMetricPoints("attendance", state.campus)
    .filter((point) => point.date.startsWith(currentYear))
    .map((point) => ({ ...point, sourceLabel: `${currentYear} attendance` }));
}

function cleanTrendPoints() {
  return [...cleanTrendHistoryPoints(), ...cleanTrendCurrentPoints()]
    .filter((point) => point.value > 0)
    .sort((a, b) => a.date.localeCompare(b.date));
}

function cleanTrendYearPoints(points, year) {
  return points.filter((point) => point.date.startsWith(String(year)));
}

function cleanTrendStats(points) {
  const currentYear = cleanTrendCurrentYear();
  const historyPoints = cleanTrendYearPoints(points, "2025");
  const currentPoints = cleanTrendYearPoints(points, currentYear);
  const historyAvg = averageNumbers(historyPoints.map((point) => point.value));
  const currentAvg = averageNumbers(currentPoints.map((point) => point.value));
  const latest = currentPoints.at(-1) || points.at(-1) || null;
  const peak = points.slice().sort((a, b) => b.value - a.value)[0] || null;
  return {
    currentYear,
    historyPoints,
    currentPoints,
    ...
    latest,
    peak,
    avgChangePct: pctChange(currentAvg, historyAvg),
  };
}

function renderCleanTrendKpis(points) {
  if (!els.cleanTrendKpis) return;
  const stats = cleanTrendStats(points);
  const kpis = [
    {
      label: "Latest Attendance",
      value: formatNumber(stats.latest?.value),
      note: stats.latest ? formatDate(stats.latest.date) : "No data",
    },
    {
      label: "2025 Sunday Avg",
      value: formatNumber(stats.historyAvg),
      note: `${formatNumber(stats.historyPoints.length)} Sundays tracked`,
    },
    {
      label: `${stats.currentYear} Sunday Avg`,
      value: formatNumber(stats.currentAvg),
      note: `${formatNumber(stats.currentPoints.length)} Sundays tracked`,
    },
    {
      label: `${stats.currentYear} vs 2025 Avg`,
      value: formatPct(stats.avgChangePct),
      valueClass: toneClass(stats.avgChangePct),
      note: "Average Sunday attendance",
    },
  ];

  els.cleanTrendKpis.innerHTML = kpis
    .map(
      (kpi) => `
        <article class="kpi">
          <div class="kpi-label">${escapeHtml(kpi.label)}</div>
          <div class="kpi-value ${kpi.valueClass || ""}">${escapeHtml(kpi.value)}</div>
...
function cleanTrendTooltipLines(point, index, points) {
  const previous = points[index - 1];
  const lines = [point.sourceLabel || ""].filter(Boolean);
  if (previous) {
    const delta = point.value - previous.value;
    const change = pctChange(point.value, previous.value);
    if (isFiniteNumber(delta) && isFiniteNumber(change)) {
      lines.push(`${formatSignedNumber(delta)} (${formatPct(change)}) from previous point`);
    }
  }
  if (point.event) lines.push(point.event);
  return lines.slice(0, 3);
}

function cleanTrendAxisTicks(points, xDate, height) {
  const first = points[0];
  const last = points.at(-1);
  const firstYear = first.date.slice(0, 4);
  const lastYear = last.date.slice(0, 4);
  const candidates = [
    { date: first.date, label: shortDate(first.date) },
    { date: `${firstYear}-07-01`, label: `Jul ${firstYear}` },
    { date: `${lastYear}-01-01`, label: lastYear },
    { date: `${lastYear}-07-01`, label: `Jul ${lastYear}` },
    { date: last.date, label: shortDate(last.date) },
  ];
  const seen = new Set();
  const ticks = candidates
    .filter((tick) => tick.date >= first.date && tick.date <= last.date)
    .filter((tick) => {
      if (seen.has(tick.date)) return false;
      seen.add(tick.date);
      return true;
    })
    .map((tick) => ({ ...tick, x: xDate(tick.date) }));
  const spacedTicks = [];
  const minGap = 95;
  for (const tick of ticks) {
    const isLast = tick.date === last.date;
    if (!spacedTicks.length) {
      spacedTicks.push(tick);
      continue;
    }
    if (isLast) {
      while (spacedTicks.length > 1 && tick.x - spacedTicks.at(-1).x < minGap) spacedTicks.pop();
      if (tick.x - spacedTicks.at(-1).x >= minGap) spacedTicks.push(tick);
      continue;
    }
    if (tick.x - spacedTicks.at(-1).x >= minGap) spacedTicks.push(tick);
  }
  ...
  return spacedTicks
    .map((tick, index, ticks) => {
      ...
      return `<text class="axis-label" x="${tick.x}" y="${height - 14}" text-anchor="${anchor}">${escapeHtml(
        tick.label,
      )}</text>`;
    })
    .join("");
...
function cleanTrendAxisSourceValues(values) {
  const sorted = values.slice().sort((a, b) => a - b);
  if (sorted.length < 6) return values;
  const median = medianNumbers(sorted);
  if (!isFiniteNumber(median) || median <= 0) return values;
  const typicalLimit = median >= 5000 ? median * 1.35 : median * 1.6;
  const typicalValues = sorted.filter((value) => value <= typicalLimit);
  return typicalValues.length >= Math.max(4, Math.ceil(sorted.length * 0.7)) ? typicalValues : values;
}

function cleanTrendTickStep(minValue, maxValue) {
  const range = Math.max(1, maxValue - minValue);
  const maxAbs = Math.max(Math.abs(minValue), Math.abs(maxValue));
  if (maxAbs >= 5000) {
    if (range <= 4500) return 1000;
    if (range <= 9000) return 2000;
    return 5000;
  }
  if (maxAbs >= 2000) return range <= 2500 ? 500 : 1000;
  if (maxAbs >= 1000) return range <= 1200 ? 250 : 500;
  if (maxAbs >= 500) return range <= 600 ? 100 : 250;
  if (maxAbs >= 200) return range <= 250 ? 50 : 100;
  return 25;
}

function cleanTrendYAxis(values) {
  const sourceValues = cleanTrendAxisSourceValues(values);
  const actualMax = Math.max(...values);
  const actualMin = Math.min(...values);
  const maxValue = Math.max(...sourceValues);
  const minValue = Math.min(...sourceValues);
  const step = cleanTrendTickStep(minValue, maxValue);
  let min = Math.max(0, Math.floor(minValue / step) * step);
  ...
  return {
    ...
    max,
    ticks,
    focused: actualMin < min || actualMax > max,
  };
}
...
function renderCleanTrendChart(points) {
  if (!els.cleanTrendChart) return;
  if (!points.length) {
    els.cleanTrendChart.innerHTML = `<div class="empty">Connect Excel to load the clean attendance trend.</div>`;
    return;
  }
  ...
  const yAxis = cleanTrendYAxis(values);
  const yMin = yAxis.min;
  const yMax = yAxis.max;
  const domainStart = parseIsoDate(points[0].date);
  const domainEnd = parseIsoDate(points.at(-1).date);
  const domainSpan = Math.max(1, domainEnd - domainStart);
  const xDate = (date) => margin.left + ((parseIsoDate(date) - domainStart) / domainSpan) * innerWidth;
  const offScaleTopY = margin.top - 12;
  const offScaleBottomY = margin.top + innerHeight + 12;
  const y = (value) => {
    if (value > yMax) return offScaleTopY;
    if (value < yMin) return offScaleBottomY;
    const scaledValue = clampNumber(value, yMin, yMax);
    return margin.top + ((yMax - scaledValue) / (yMax - yMin)) * innerHeight;
  };
  const linePath = points
    .map((point, index) => `${index === 0 ? "M" : "L"} ${xDate(point.date)} ${y(point.value)}`)
    .join(" ");
  const gridValues = yAxis.ticks;
  const tooltipWidth = 270;
  const yearBoundary = `${cleanTrendCurrentYear()}-01-01`;
  const showYearBoundary = yearBoundary > points[0].date && yearBoundary < points.at(-1).date;

  els.cleanTrendChart.innerHTML = `
    <svg viewBox="0 0 ${width} ${height}" role="img">
      ${gridValues
        .map((value) => {
          const yy = y(value);
          return `
            <line class="grid-line" x1="${margin.left}" y1="${yy}" x2="${width - margin.right}" y2="${yy}"></line>
            <text class="axis-label" x="${margin.left - 10}" y="${yy + 4}" text-anchor="end">${formatCompactAxisValue(value)}</text>
          `;
        })
        .join("")}
      ${
        showYearBoundary
          ? `
            <line class="clean-trend-divider" x1="${xDate(yearBoundary)}" y1="${margin.top}" x2="${xDate(
              yearBoundary,
            )}" y2="${margin.top + innerHeight}"></line>
            <text class="axis-label clean-trend-divider-label" x="${xDate(yearBoundary) + 8}" y="${
              margin.top + 14
            }">${escapeHtml(cleanTrendCurrentYear())}</text>
          `
          : ""
      }
      <path class="series-line clean-trend-line" d="${linePath}"></path>
      ${points
        .map((point, index) => {
          const cx = xDate(point.date);
          const cy = y(point.value);
          const noteLines = cleanTrendTooltipLines(point, index, points);
          if (point.value > yMax) noteLines.push("Above focused scale");
          if (point.value < yMin) noteLines.push("Below focused scale");
          const previous = points[index - 1];
          const pointChange = previous ? pointChangeSummary(point, index, points) : null;
          const tooltipHeight = 54 + noteLines.length * 14;
          const tooltipX = Math.min(Math.max(cx - tooltipWidth / 2, margin.left - 24), width - margin.right - tooltipWidth + 24);
          const tooltipY = Math.max(cy - tooltipHeight - 18, margin.top - 10);
          const isEvent = Boolean(point.event) || point.eventType !== "normal";
          const isAxisClamped = point.value > yMax || point.value < yMin;
          const offScaleLabel = point.value > yMax ? formatCompactAxisValue(point.value) : null;
          return `
            <g class="chart-point-tooltip" data-clean-tooltip-index="${index}" transform="translate(${tooltipX} ${tooltipY})">
              <rect width="${tooltipWidth}" height="${tooltipHeight}" rx="7"></rect>
              <text x="12" y="20">${escapeHtml(formatDate(point.date))}</text>
              <text class="tooltip-number" x="12" y="38">
                <tspan>${escapeHtml(formatNumber(point.value))}</tspan>
                ${
                  pointChange
            ...
            <circle class="point ${isEvent ? "event" : ""} ${isAxisClamped ? "axis-clamped" : ""}" data-clean-point-index="${index}" tabindex="0" aria-label="${escapeHtml(
              `${formatDate(point.date)} ${formatNumber(point.value)}${point.event ? ` ${point.event}` : ""}`,
            )}" cx="${cx}" cy="${cy}" r="${isEvent ? 4.5 : 3.5}">
              <title>${escapeHtml(formatDate(point.date))}: ${escapeHtml(formatNumber(point.value))}${
                point.event ? `, ${escapeHtml(point.event)}` : ""
              }</title>
            </circle>
            ${
        ...
        .join("")}
      ${cleanTrendAxisTicks(points, xDate, height)}
    </svg>
  `;
  wireCleanTrendTooltips();
}

function wireCleanTrendTooltips() {
  let pinnedIndex = null;
  const points = els.cleanTrendChart.querySelectorAll("[data-clean-point-index]");
  const tooltips = els.cleanTrendChart.querySelectorAll("[data-clean-tooltip-index]");

  const showTooltip = (index, pinned = false) => {
    if (pinned) pinnedIndex = index;
    for (const tooltip of tooltips) {
      tooltip.classList.toggle("active", tooltip.dataset.cleanTooltipIndex === index);
    }
  };
  ...
  for (const point of points) {
    point.addEventListener("mouseenter", () => showTooltip(point.dataset.cleanPointIndex));
    point.addEventListener("focus", () => showTooltip(point.dataset.cleanPointIndex));
    point.addEventListener("mouseleave", hideTooltip);
    point.addEventListener("blur", hideTooltip);
    point.addEventListener("click", () => {
      pinnedIndex = pinnedIndex === point.dataset.cleanPointIndex ? null : point.dataset.cleanPointIndex;
      showTooltip(point.dataset.cleanPointIndex, pinnedIndex !== null);
    });
  }
}

function renderCleanTrend() {
  const points = cleanTrendPoints();
  const stats = cleanTrendStats(points);
  const campusLabel = state.campus === "All Campuses" ? "All Campuses" : state.campus;
  if (els.cleanTrendTitle) {
    els.cleanTrendTitle.textContent =
      state.campus === "All Campuses" ? "All Campuses 2 Year Trend" : `${state.campus} 2 Year Trend`;
  }
  if (els.cleanTrendMeta) {
    const eventLabel = state.showEvents ? "Event weeks included" : "Regular Sundays only";
    els.cleanTrendMeta.textContent = `${campusLabel} · 2025 to ${stats.currentYear} · ${formatNumber(
      points.length,
    )} Sundays · ${eventLabel}`;
  }
  renderCleanTrendKpis(points);
  renderCleanTrendChart(points);
}
...
function teamLeadVacancyVisibleInMonth(row, month) {
  if (!month) return true;
  const filledMonth = monthKeyFromDateLike(row.filledDate);
  const effectiveMonth = teamLeadVacancyEffectiveMonth(row);
  if (filledMonth && month >= filledMonth) return false;
  if (effectiveMonth) return month >= effectiveMonth;
  if (row.month) return row.month === month;
  return true;
}

function teamLeadVacancyStartedByMonth(row, month) {
  if (!month) return true;
  const effectiveMonth = teamLeadVacancyEffectiveMonth(row);
  if (effectiveMonth) return month >= effectiveMonth;
  if (row.month) return row.month === month;
...
function renderExecHealthPrintHeader(report) {
  ...
  els.execHealthPrintTitle.textContent = execHealthDocumentTitle(report);
  els.execHealthPrintMeta.textContent = `${monthLabel} · ${workbookLabel}`;
}

function reportCampusTitle() {
  return state.campus === "All Campuses" ? "All Campuses" : state.campus;
}

function reportMonthYearTitle(month) {
  return month ? formatMonth(month) : "Current Month";
}

function execHealthDocumentTitle(report) {
  return `${reportCampusTitle()} ${reportMonthYearTitle(report?.month)} Exec Health Report`;
}

function shouldTightenExecHealthPrint() {
  return ["ubc", "fallschurch"].includes(normalizeText(state.campus));
}

function renderHealthPrintHeader(report) {
  if (!els.healthPrintTitle || !els.healthPrintMeta) return;
  const campusLabel = reportCampusTitle();
  const monthLabel = report.month ? healthMonthDisplay(report.month) : "Current month";
  const workbookLabel = els.sourceName?.textContent?.trim() || "Campus dashboard";
...
function printExecHealthReport() {
  ...
  const printTitle = execHealthDocumentTitle(report);
  const tightPrint = shouldTightenExecHealthPrint();
  renderExecHealthPrintHeader(report);
  document.title = printTitle;
  document.body.classList.add("print-health-report", "print-exec-health-report");
  document.body.classList.toggle("print-tight-exec-report", tightPrint);
  window.setTimeout(() => {
    window.print();
    window.setTimeout(() => {
      document.body.classList.remove("print-health-report", "print-exec-health-report", "print-tight-exec-report");
...
function campusAliasTerms(campus) {
  const aliases = {
    ...
    BWI: ["bwi"],
    UBC: ["ubc", "bc", "baltimorecounty", "baltimore county"],
  };
...
function bigFiveEventKey(value) {
  const normalized = normalizeText(value);
  if (!normalized) return "";
  if (normalized.includes("welcomehome") || normalized === "whs" || normalized.includes("whs")) {
    return "welcomehomesunday";
  }
  if (normalized.includes("easter")) return "easter";
  if (normalized.includes("christmas") || normalized.includes("eoy")) return "christmaseoy";
  if (normalized.includes("relationship")) return "relationshipseries";
  if (normalized.includes("atthemovies")) return "atthemovies";
  return normalized;
}

function bigFiveEventMatches(left, right) {
  const leftKey = bigFiveEventKey(left);
  const rightKey = bigFiveEventKey(right);
  return Boolean(leftKey && rightKey && (leftKey === rightKey || leftKey.includes(rightKey) || rightKey.includes(leftKey)));
}

function serviceSortValue(label) {
...
function serviceTotals(rows) {
  const total = sumNumbers(rows.map((row) => row.total));
  const auditorium = sumNumbers(rows.map((row) => row.auditorium));
  return {
    total,
    auditorium,
    auditoriumShare: total && auditorium !== null ? (auditorium / total) * 100 : null,
  };
}

function bigFiveServiceRowsForRecord(record) {
  if (!record) return [];
  return (data.bigFive.serviceBreakdown || [])
    .filter((row) => Number(row.year) === Number(record.year))
    .filter((row) => bigFiveEventMatches(row.event, record.event) || bigFiveEventMatches(row.event, record.eventLabel))
    .filter((row) => state.campus === "All Campuses" || campusNameMatches(row.campus, state.campus))
    .filter((row) => row.service && (isFiniteNumber(row.total) || isFiniteNumber(row.auditorium)))
    .sort(
      (a, b) =>
        serviceSortValue(a.service) - serviceSortValue(b.service) ||
        String(a.campus || "").localeCompare(String(b.campus || "")),
    );
}

function groupedServiceRows(rows, key) {
  const grouped = new Map();
  for (const row of rows) {
...
function serviceRatioText(row) {
  return row.total && isFiniteNumber(row.auditorium) ? formatHealthPct((row.auditorium / row.total) * 100) : "--";
}

function renderBigFiveServiceSummary(rows) {
  const totals = serviceTotals(rows);
  const serviceGroups = groupedServiceRows(rows, "service").sort((a, b) => serviceSortValue(a.label) - serviceSortValue(b.label));
  const cards = [
    {
      label: "Total Attendance",
      value: formatNumber(totals.total),
      note: "All tracked services",
      className: "is-total",
    },
    {
      label: "Auditorium Attendance",
      value: formatNumber(totals.auditorium),
      note: "Auditorium only",
      className: "is-total",
    },
    {
      label: "Auditorium Share",
      value: serviceRatioText(totals),
      note: "Auditorium vs total",
      className: "is-total",
    },
    {
      label: "Services Tracked",
      value: formatNumber(serviceGroups.length),
      note: state.campus === "All Campuses" ? "Across campuses" : state.campus,
      className: "is-total",
    },
    ...serviceGroups.map((group) => ({
      label: group.label,
      value: formatNumber(group.total),
      note: `${formatNumber(group.auditorium)} auditorium · ${serviceRatioText(group)}`,
    })),
  ];

  els.bigFiveServiceSummary.innerHTML = cards
    .map(
      (card) => `
        <article class="big-five-service-card ${card.className || ""}">
...
function renderBigFiveServiceTable(rows) {
  if (state.campus === "All Campuses") {
    const campusGroups = groupedServiceRows(rows, "campus").sort((a, b) => b.total - a.total);
    els.bigFiveServiceTableHead.innerHTML = ["Campus", "Total", "Auditorium", "Auditorium Share", "Services"]
      .map((header) => `<th>${header}</th>`)
      .join("");
    els.bigFiveServiceTable.innerHTML = campusGroups
      .map((campus) => {
        const services = campus.rows
          .slice()
          .sort((a, b) => serviceSortValue(a.service) - serviceSortValue(b.service))
          .map(
            (row) => `
              <span class="service-chip">
                <strong>${escapeHtml(row.service)}</strong>
                ${formatNumber(row.total)} / ${formatNumber(row.auditorium)} aud.
              </span>
            `,
          )
          .join("");
        return `
          <tr>
            <td>${escapeHtml(campus.label)}</td>
            <td>${formatNumber(campus.total)}</td>
            <td>${formatNumber(campus.auditorium)}</td>
            <td>${serviceRatioText(campus)}</td>
            <td><div class="service-chip-list">${services}</div></td>
          </tr>
        `;
      })
      .join("");
  ...
  els.bigFiveServiceTableHead.innerHTML = ["Service", "Total", "Auditorium", "Auditorium Share"]
    .map((header) => `<th>${header}</th>`)
    .join("");
  els.bigFiveServiceTable.innerHTML = rows
    .map(
      (row) => `
        <tr>
          <td>${escapeHtml(row.service)}</td>
          <td>${formatNumber(row.total)}</td>
          <td>${formatNumber(row.auditorium)}</td>
          <td>${serviceRatioText(row)}</td>
        </tr>
      `,
    )
    .join("");
...
function renderBigFiveServiceBreakdown(records) {
  if (
    !els.bigFiveServicePanel ||
    !els.bigFiveServiceTitle ||
    !els.bigFiveServiceMeta ||
    !els.bigFiveServiceSummary ||
    !els.bigFiveServiceTableHead ||
    !els.bigFiveServiceTable
  ) {
    return;
  }
  ...
  const selected = selectedBigFiveRecord(records);
  if (!selected) {
    els.bigFiveServicePanel.classList.add("is-hidden");
    return;
  }

  const rows = bigFiveServiceRowsForRecord(selected);
  els.bigFiveServicePanel.classList.remove("is-hidden");
  els.bigFiveServiceTitle.textContent = `${selected.year} ${selected.eventLabel} Service Breakdown`;

  if (!rows.length) {
    els.bigFiveServiceMeta.textContent = "Waiting for service-level source data";
    els.bigFiveServiceSummary.innerHTML = `
      <div class="empty big-five-service-empty">
        No service breakdown is loaded for ${escapeHtml(selected.year)} ${escapeHtml(
          selected.eventLabel,
        )} yet. Add rows to a Big 5 Service Breakdown sheet with Event, Year, Campus, Service Time, Total Attendance, and Auditorium Attendance.
      </div>
    `;
    els.bigFiveServiceTableHead.innerHTML = "";
    els.bigFiveServiceTable.innerHTML = "";
    return;
  }

  const sources = Array.from(new Set(rows.map((row) => row.source).filter(Boolean)));
  const campusCount = new Set(rows.map((row) => row.campus).filter(Boolean)).size;
  const serviceCount = new Set(rows.map((row) => row.service).filter(Boolean)).size;
  els.bigFiveServiceMeta.textContent = `${sources[0] || "Service-level source"} · ${formatNumber(campusCount)} ${
    campusCount === 1 ? "campus" : "campuses"
  } · ${formatNumber(serviceCount)} ${serviceCount === 1 ? "service" : "services"}`;

  renderBigFiveServiceSummary(rows);
  renderBigFiveServiceTable(rows);
}
...
function renderBigFive() {
  ...
  renderBigFiveServiceBreakdown(records);
...
function isCleanTrendSelected() {
  return state.metric === "cleanTrend";
}
...
function renderMetricPanels() {
  ...
  const showCleanTrend = isCleanTrendSelected();
  ...
  for (const panel of els.metricPanels) {
    ...
    const visible =
      ...
      (mode === "clean-trend" && showCleanTrend) ||
  ...
  els.eventToggleWrapper?.classList.toggle("is-hidden", !(showStandard || showCleanTrend));
}
